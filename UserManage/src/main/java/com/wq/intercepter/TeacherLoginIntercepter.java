 ...
 * @author liuzhibo
 */
public class TeacherLoginIntercepter extends HandlerInterceptorAdapter {
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
        ...
        if (isNeedLogin(requestURI)) {
            ...
            Student student = (Student) session.getAttribute(LoginConstant.TEACHER_SESSION);
            if (student != null) {
                // 用户已经登录，返回true
                return true;
            } else {
                //没有登陆，跳转至登录页面
                response.sendRedirect(RequestUtil.getBasePath(request) + "teacher/login");
                return false;
            }
        } else {
            return true;
